...
RUFFEOF
  ...
  if (cd "$REPO_DIR" && "$RUFF" format --check examples/ --config "$WORK/ruff.toml" >/dev/null 2>&1); then
...
if grep -q 'globus-connect-personal' "$SCRIPT_DIR/onboard.sh"; then
  ok "explains the per-machine Globus endpoint"; else bad "personal endpoint step missing"; fi
if grep -q 'HOME DIRECTORY' "$SCRIPT_DIR/onboard.sh"; then
  ok "warns that only home is shared by default"; else bad "shared-path warning missing"; fi
if grep -q 'GLOBUS_LAPTOP=\.' "$SCRIPT_DIR/onboard.sh"; then
  ok "detects whether this machine is an endpoint"; else bad "no endpoint detection"; fi
